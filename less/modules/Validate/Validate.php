...
class Validate
{
	...
	{
		if( Less::option('validate') )
		{					
			# Get the validator options from the config
			$validator_options = Less::$config['Validate']['options'];
			
			# Add our options
			$validator_options['text'] = Less::$output;
			...
			if (!empty($buffer))
			{
			    ...
			    if($is_valid == "false")
			    {
			    	...
			    	foreach($errors->merrorlist->merror as $key => $error)
			    	{
			    		...
			    		Less::log("Validation Error on line {$line} near {$near} => {$message}",1);
			    	}
			    }
			}
		}
