 ...
 * LessPHP
 *
 * LessPHP is a CSS compiler and preprocessor that allows you to extend
...
class Less extends Less_Utils
{
	...
	public static function parse( $files, $config, $options = array(), $display = false )
	{
		...
		{			
			...
			Less_Benchmark::start('system.flags');
			
			# Get the flags from each of the loaded modules.
			$flags = (self::$flags === false) ? array() : self::flags();
			
			# Time it takes to get the flags
			Less_Benchmark::stop('system.flags');
			
			# The final, combined CSS file in the cache
			$combined = md5(serialize(array($files,$flags))) . '.less';
			
			/**
			 * Check if we should use the combined cache right now and skip unneeded processing
			 */
			if(LESS_PRODUCTION === true AND Less_Cache::exists($combined) AND Less_Cache::is_fresh($combined))
			{
				Less::$output = Less_Cache::open($combined);
			}
			
			if(Less::$output === null)
			{
				# We're processing the files
				Less_Benchmark::start('system.check_files');
	
				foreach($files as $file)
				{
					# The time to process a single file
					Less_Benchmark::start('system.file.' . basename($file));
					
					# Make sure this file is allowed
					if(substr($file, 0, 4) == "http" OR substr($file, -5, 5) != ".less")
					{
					...
					{
						$cached_file = str_replace('.less','.css',$file);
					}
					 ...
					 */
					$modified = (LESS_PRODUCTION) ? Less_Cache::modified($cached_file) : 0;
		
					/**
					 * If the CSS file has been changed, or the cached version doesn't exist
					 */			
					if(!Less_Cache::exists($cached_file) OR $modified < filemtime($request))
					{
						Less_Cache::write( Less::process($request), $cached_file );
						Less_Cache::remove($combined);
					}
		
					$css .= Less_Cache::open($cached_file);
					
					# The time it's taken to process this file
					Less_Benchmark::stop('system.file.' . basename($file));
				}
				 ...
				 */
				if(!Less_Cache::exists($combined))
				{
					Less_Cache::write(self::$output,$combined);
				}
				
				# The time it takes to process the files
				Less_Benchmark::stop('system.check_files');
			
				/**
				 * Hook to modify what is sent to the browser
				 */
				if(LESS_PRODUCTION === false) Less::hook('display');
			}
			...
			$modified = Less_Cache::modified($combined);
			$lifetime = (LESS_PRODUCTION === true) ? $config['cache_lifetime'] : 0;
			...
			if($display === true)
			{
				Less::render(Less::$output,$config['gzip_compression']);
			}
			
			# Benchmark will do the entire run from start to finish
			Less_Benchmark::stop('system');
		}
		...
		catch( Exception $e )
		{
			 ...
			 */
			if(LESS_PRODUCTION === false && $display === true)
			{
				Less::send_headers();
				require Less::find_file('less_error.php','views');
			}
		}
		
		# Log the final execution time
		#$benchmark = Less_Benchmark::get('system');
		#Less_Log::log('Total Execution - ' . $benchmark['time']);

		# Save the logs and exit 
		Less_Event::run('system.shutdown');
	...
	public static function setup($config) 
	{
		 ...
		 */
		if(!defined('LESS_SYSPATH') && !defined('LESS_DOCROOT'))
		{
			define('LESS_SYSPATH', self::fix_path($config['system']));
			define('LESS_DOCROOT', $config['document_root']);
			define('LESS_URLPATH', str_replace(LESS_DOCROOT, '',LESS_SYSPATH));
		}

		/**
		 * Add include paths for finding files
		 */
		Less::add_include_path(LESS_SYSPATH,LESS_DOCROOT);
	
		/**
		 * Tell the cache where to save files and for how long to keep them for
		 */
		Less_Cache::setup( Less::fix_path($config['cache']), $config['cache_lifetime'] );
		...
		{
			Less_Log::log_directory(LESS_SYSPATH.'logs');			
			Less_Event::add('system.shutdown', array('Less_Log','save'));
		}

		/**
		 * Load each of the modules
		 */
		foreach(Less::list_files(LESS_SYSPATH.'modules') as $module)
		{
			$name = basename($module);
			$module_config = LESS_SYSPATH.'config/' . $name . '.php';
		 ...
		 */
		Less_Event::add('system.shutdown', array('Less','shutdown'));
	}
	...
	public static function add_include_path($path)
	{
		...
		{
			self::$include_paths[] = Less_Utils::fix_path($path);
		}
	}
