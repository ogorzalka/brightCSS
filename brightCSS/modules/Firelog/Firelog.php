 ...
 * adapted from CSScaffold by Anthony Short
 */
class Firelog
{
	/**
	...
	/**
	 * Outputs the benchamrks from Less to Firebug
	 *
	 * @return void
	 */
	public static function benchmark()
	{
		$system 	= bCSS_Benchmark::get('system');
		$flags 		= bCSS_Benchmark::get('system.flags');
		$check		= bCSS_Benchmark::get('system.check_files');
	...
	public static function display()
	{
		...
		if(Less::$config['Firelog']['file_information'] === true)
		{
			self::_file(Less::$css->file,'File Information');
		}

		# Constants
		if(Less::$config['Firelog']['constants'] === true && class_exists('Constants') && Constants::$constants)
		{
			$table = array();
		...
		# Mixins
		if(Less::$config['Firelog']['mixins'] === true && class_exists('Mixins') && Mixins::$mixins)
		{
			$table = array();
		...
		# Included files
		if(Less::$config['Firelog']['included_files'] === true && class_exists('Import') && Import::$loaded)
		{
			self::_group('Included Files', Import::$loaded, 3);
		}
		
		# Flags
		if(Less::$config['Firelog']['flags'] === true)
		{
			self::_group('Flags', (Less::flags()) ? Less::flags() : 'No flags are set');
		}
		
		# Include Paths
		if(Less::$config['Firelog']['include_paths'] === true)
		{
			self::_group('Include Paths', Less::include_paths());
		}

		# Error Log
		if(Less::$config['Firelog']['error_log'] === true)
		{
			foreach(bCSS_Log::$log as $type => $value)
			{
		...
		# Custom Functions		
		/*
		if(Less::$config['Firelog']['custom_functions'] === true && class_exists('Extensions') && Extensions::$functions)
		{
			self::_group('Custom Functions', Extensions::$functions);
		}
		
		# Custom Properties
		if(Less::$config['Firelog']['custom_properties'] === true && class_exists('Extensions') && Extensions::$properties)
		{
		...
		*/
		...
		# Gradients
		if(Less::$config['Firelog']['gradients'] === true && class_exists('Gradient') && Gradient::$gradients)
		{
			$table = array();
		...
		/*
		# Layout module
		if(Less::$config['Firelog']['layout'] === true && isset(Layout::$columns))
		{
			FB::group('Layout');
		...
		*/
		...
		# Validation Errors
		if(Less::$config['Firelog']['validation_errors'] === true && Validate::$errors)
		{
	...
	private static function _file($file,$name = false)
	{
		...
		$table[] = array('Compressed Size', bCSS_Utils::readable_size($contents));
		$table[] = array('Gzipped Size', bCSS_Utils::readable_size($gzipped));
		FB::table($name,$table);
	}
}