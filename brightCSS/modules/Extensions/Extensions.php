...
class Extensions
{
	...
	{
		$files = Less::list_files($location,true);
		...
		foreach ($files as $path)
		{
			...
			/**
			 * The name of the property that can be used in Less CSS
			 */
			 ...
			 */
			$callback = 'bCSS_'.str_replace('-','_',$extension_name);
			 ...
			 */
			if($found = Less::$css->$function($extension_name))
			{
				...
				foreach($originals as $key => $value)
				{
					...
					if($result === false)
					{
						Less::error('Invalid Extension Syntax - <strong>' . $originals[$key] . '</strong>');
					}
					
					// Just replace the first match if they are unique
					elseif($unique === true)
					{
						$pos = strpos(Less::$css->string,$originals[$key]);

						if($pos !== false)
						{
						    Less::$css->string = substr_replace(Less::$css->string,$result,$pos,strlen($originals[$key]));
						}
					}
					else
					{
						Less::$css->string = str_replace($originals[$key],$result,Less::$css->string);
					}
				}
			}
		}
